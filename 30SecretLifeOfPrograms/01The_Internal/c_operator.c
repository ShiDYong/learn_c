...
void test_logical_XOR() {
    /*
     * 尽管使用 ^ 运算符进行奇偶判断在某些特定情况下可能工作，但这种方法不可靠且容易引起混淆。
     * 建议使用模运算 % 或按位与运算 & 来实现更为直观且可靠的奇偶判断
     * 理解 num ^ 1 表达式的行为：
    对于偶数，最后一位（二进制表示的最低位）是0。
    对于奇数，最后一位是1。
    将任何数与1进行异或操作，会翻转这个数的最后一位。例如：
    如果 num 是偶数，num ^ 1 将把最后一位0变为1。
    如果 num 是奇数，num ^ 1 将把最后一位1变为0
     */
    int num = 5;
//    if (num ^ 1) {
//        printf("%d if odd\n", num);
//    } else
     ...
     * num ^ 1 等价于 5 ^ 1。
        ...
        0101（5的二进制）
        0001（1的二进制）
        结果是 0100，即 4，如果将num值改为4，结果也是一样的
        就不准确了：
        num ^ 1 等价于 4 ^ 1。
        二进制计算：
        0100（4的二进制）
